...
//            0 => 'Name'
        ...
        $query = "SELECT count(sno) as total FROM GuardAttendance where AttDate between '2021-11-01' and '2021-11-30' and EmpCode ='$emp_code'";
        $result = sqlsrv_query($this->conn_1, $query);
        $query = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC);
        $totalData = (count($query) > 0) ? $query['total'] : 0;
        $totalFiltered = $totalData;
        ...
        $month_last_day = date('t');
        $sql = "SELECT InTime,OutTime,CONVERT(varchar, AttDate, 23) as AttDate,DATEDIFF(minute,InTime,OutTime)-30 as working_hrs FROM GuardAttendance where AttDate between '2021-11-01' and '2021-11-30' and EmpCode ='$emp_code'";
